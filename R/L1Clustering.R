...
.clusterProfileLinearise <- function(resObjList, outer_struct, se, Eta) {
  ...
  for (si in seq_len(n)) {
    ...
    full <- c(outer_struct, base_eta); full[en] <- Eta[si, ]
    o <- tryCatch(resObjList[[subjects[si]]](full, deriv = TRUE), error = function(e) NULL)
...
.clusterSolve <- function(resObjList, outer_struct, penalty, lambda,
  ...
  Fval <- function(Eta) {
    dv <- 0
    for (s in subjects) {
      v <- tryCatch(resObjList[[s]](fullFor(s, Eta[s, ]), deriv = FALSE)$value,
  ...
  linearise <- function(Eta) {
    G_ <- H_ <- matrix(0, n, K, dimnames = list(subjects, eta_cols)); ok <- TRUE
    for (si in seq_len(n)) {
      o <- tryCatch(resObjList[[subjects[si]]](fullFor(subjects[si], Eta[si, ]),
                                               deriv = TRUE), error = function(e) NULL)
      if (is.null(o)) { ok <- FALSE; break }
      en <- as.character(se[subjects[si], ])
      G_[si, ] <- o$gradient[en]
      H_[si, ] <- pmax(diag(o$hessian[en, en, drop = FALSE]), 0)  # Gauss-Newton Hd >= 0
    }
    list(grad = G_, Hd = H_, ok = ok)
  }
...
.clusterMap <- function(rec, resObjList, free, fixed, lam_name,
  ...
  cm1_obj <- function(theta_struct, Eta) {
    ...
    for (cc in conds) {
      ...
      o <- tryCatch(resObjList[[cc]](full, deriv = TRUE), error = function(e) NULL)
...
.selectCluster <- function(obj, init, fixed = NULL, control = list(),
  ...
  resObjList <- stats::setNames(lapply(all_conditions, function(s) {
    ...
    normL2(d, rec$prdfn, errmodel = rec$errfn, use.bessel = FALSE)
  }), all_conditions)
  ...
  fitsList <- if (cores > 1L && fits > 1L)
                parallel::mclapply(seq_len(fits), fitOne, mc.cores = cores)
  ...
  if (joint && length(eta_cols) > 1L) {
    ...
    Omega <- lapply(subjects, function(s) {
      en <- as.character(se[s, ])
      full <- c(struct_hat, fixed, base_eta); full[en] <- sol$etahat[s, ]
      resObjList[[s]](full, deriv = TRUE)$hessian[en, en, drop = FALSE]
    })
...
.clusterSaem <- function(rec, resObjList, free, fixed, lam_name,
  ...
  cm1_obj <- function(th, E) {
    ...
    HH <- matrix(0, length(struct_names), length(struct_names),
                 dimnames = list(struct_names, struct_names))
    for (si in seq_len(n)) {
      o <- resObjList[[subjects[si]]](fullFor(os, subjects[si], E[si, ]), deriv = TRUE)
    ...
    for (rc in ref_conditions) {
      o <- resObjList[[rc]](c(os, base_eta), deriv = TRUE); sn <- intersect(struct_names, names(o$gradient))
  ...
  dvc  <- vapply(subjects, function(s) resObjList[[s]](fullFor(os0, s, Eta[match(s, subjects), ]), deriv = FALSE)$value, 0.0)
