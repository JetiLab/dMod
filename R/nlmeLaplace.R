...
## normL2 built on this subject's data alone (normL2 fixes its conditions from the
## data at build time and ignores a call-time `conditions =`, so a shared all-subject
## objfn would return the summed value and pollute every subject's marginal). The
## outer (structural + error) parameters are held at `outer_struct`; all etas start
## at 0 and only subject i's block varies. Value/gradient/hessian follow dMod's
## -2 log L convention, so they feed .laplaceSubjectMarginal() directly.
.laplaceSubjectObjfun <- function(resObj_s, outer_struct, all_eta_names,
  ...
  function(p, ...) {
    full <- base_full
    full[eta_names_i] <- p[eta_names_i]
    o <- resObj_s(full, deriv = TRUE)
    list(value    = o$value,
...
##   correction = FALSE (FOCE): d(-2logL)/d theta = data gradient at the mode plus
...
##   correction = TRUE (FOCEI): the EXACT total derivative of the Laplace-approx
##     marginal value, d M/d theta = explicit(eta frozen) + implicit mode shift
##     (d M/d etahat)(d etahat/d theta), including lambda. Uses cheap frozen
##     central differences of the exact data-Hessian diagonal (no mode re-solve,
##     no deriv2); matches the moving-mode FD gradient to ~1e-6.
...
.laplaceOuterEval <- function(theta, rec, resObjList, targets_by_subject,
  ...
  for (ii in seq_along(subjects)) {
    s           <- subjects[ii]
    eta_names_i <- as.character(se[s, ])
    objfun_i    <- .laplaceSubjectObjfun(resObjList[[s]], outer_struct, all_eta,
                                         eta_names_i)
    ...
    eta0 <- if (!is.null(warm) && !is.null(warm[[s]]))
      warm[[s]] else setNames(rep(0, length(eta_names_i)), eta_names_i)
    eta0 <- eta0[eta_names_i]
    tg   <- targets_by_subject[[s]][eta_names_i]
    ...
    if (grad) {
      full_mode <- c(outer_struct, setNames(rep(0, length(all_eta)), all_eta))
      full_mode[eta_names_i] <- sm$etahat
      o_full  <- resObjList[[s]](full_mode, deriv = TRUE)
      ...
      if (!correction) {
        ...
        ## FOCE gap (structural + lambda) to FD-accuracy. d Hd_k/d{theta,eta} are
        ## cheap frozen central differences of the exact data-Hessian diagonal
        ## (no mode re-solve). Matches the moving-mode FD gradient to ~1e-6.
        dI_dm <- nl$dlogI_dm; dI_da <- nl$dlogI_da; dI_dl <- nl$dlogI_dlambda
        HdDiag <- function(fp)
          diag(resObjList[[s]](fp, deriv = TRUE)$hessian[eta_names_i, eta_names_i,
                                                         drop = FALSE])
        dHd_dth <- matrix(0, K, length(struct_names),
                          dimnames = list(eta_names_i, struct_names))
        for (q in struct_names) {
          hq <- 1e-5 * max(abs(full_mode[[q]]), 1)
          fp <- full_mode; fp[[q]] <- fp[[q]] + hq
          fm <- full_mode; fm[[q]] <- fm[[q]] - hq
          dHd_dth[, q] <- (HdDiag(fp) - HdDiag(fm)) / (2 * hq)
        }
        dHd_deta <- matrix(0, K, K, dimnames = list(eta_names_i, eta_names_i))
        for (l in seq_len(K)) {
          el <- eta_names_i[l]
          hl <- 1e-5 * max(abs(full_mode[[el]]), 1)
          fp <- full_mode; fp[[el]] <- fp[[el]] + hl
          fm <- full_mode; fm[[el]] <- fm[[el]] - hl
          dHd_deta[, l] <- (HdDiag(fp) - HdDiag(fm)) / (2 * hl)
        }
  ...
  for (rc in ref_conditions) {
    ...
    o_ref    <- resObjList[[rc]](full_ref, deriv = grad)
...
#'   finite difference per structural parameter; set `FALSE` for the plain FOCE
#'   gradient); and `saem` (a list of `nBurnin`, `nEM`, `nMcmc`, `stepsize`, `cm1`
#'   for `method = "saem"`).
...
.fitLaplace <- function(obj, init, fixed = NULL, method = c("focei", "saem"),
  ...
  ## normL2 fixes its conditions from the data at build time (objClass.R:
  ## intersect(x, data)); a single all-subject objfn ignores a call-time
  ## conditions= and would return the summed value, polluting every per-subject
  ## marginal (and inflating sigma). Conditions WITH a random effect (in the
  ...
  resObjList <- setNames(lapply(all_conditions, function(s) {
    ...
    normL2(data_s, rec$prdfn, errmodel = rec$errfn, use.bessel = FALSE)
  }), all_conditions)
  ...
  ## FOCEI normal-Laplace volume correction in the CM-1 gradient (default on;
  ## exact for the error-model parameters). Costs a frozen-mode central difference
  ## of the data Hessian per structural parameter per subject; set FALSE for the
  ## plain FOCE gradient when running many fits (.msfitLaplace / .selectLaplace) for speed.
  useCorrection <- if (is.null(control$correction)) TRUE else isTRUE(control$correction)
...
.msfitLaplace <- function(obj, center, fixed = NULL, method = c("focei", "saem"),
  ...
  results <- if (cores > 1L)
    parallel::mclapply(seq_len(fits), doOne, mc.cores = cores)
...
.selectLaplace <- function(obj, center, fixed = NULL, method = c("focei", "saem"),
  ...
  evalSupport <- function(j) {
    ...
    obj_j <- normL2(rec$data, rec$prdfn, errmodel = rec$errfn,
                    use.bessel = FALSE) + constraintL1(pen_j)
...
.laplaceSubjectMarginal <- function(objfun, eta0, targets, lambda,
  ...
  list(value = m2ll, etahat = etahat, a = a, m = m, m_shift = m_shift,
       grad = gd, Hd = Hd, nl = nl, converged = isTRUE(fit$converged))
}
