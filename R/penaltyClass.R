...
constraintL1 <- function(penalty, attr.name = "prior_l1", condition = NULL) {
  ...
  myfn <- function(..., fixed = NULL, deriv = TRUE, deriv2 = FALSE,
    ...
    zero <- objlist(value = 0,
                    gradient = setNames(numeric(length(p)), names(p)),
                    hessian  = matrix(0, length(p), length(p),
                                      dimnames = list(names(p), names(p))))
    if (!all(parnames %in% names(allp))) return(zero)

    gr  <- setNames(numeric(length(p)), names(p))
    val <- 0
    for (tm in terms) {
      lambda <- as.numeric(allp[[tm$lambda]])   # this term's strength
      a  <- as.numeric(allp[[tm$i]])
      b  <- if (is.na(tm$j)) tm$target else as.numeric(allp[[tm$j]])
      d  <- a - b
      ad <- abs(d)
      val <- val + lambda * ad
      s   <- if (d > 0) 1 else if (d < 0) -1 else 0
      if (tm$i %in% names(gr)) gr[tm$i] <- gr[tm$i] + lambda * s
      if (!is.na(tm$j) && tm$j %in% names(gr)) gr[tm$j] <- gr[tm$j] - lambda * s
      if (tm$lambda %in% names(gr)) gr[tm$lambda] <- gr[tm$lambda] + ad
    }
