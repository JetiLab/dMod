...
Rcpp::List resCpp(Rcpp::DataFrame data, SEXP out, Rcpp::Nullable<Rcpp::DataFrame> err = R_NilValue, std::string optBLOQ = "none") {
  ...
  if (!err.isNull() && any_na_sigma) {
    ...
    // Match err predictions similar to out predictions
    if (err_df.containsElementNamed("time") && err_df.containsElementNamed("name")) {
      ...
      } else if (err_df.containsElementNamed("value")) {
        eval = err_df["value"];
      } else {
        stop("`err` data.frame must have column 'prediction' or 'value'.");
      }
      ...
      // Wide format error model - similar to out handling
      // (Implementation similar to out wide format - omitted for brevity)
      stop("Wide format error model not yet implemented in this version.");
    }
  }
