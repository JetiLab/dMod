...
public:
  ...
  /// Set the minimum error change for stagnation detection.
  /// If the error change is below this value for maxStagnationIterations consecutive iterations, the optimizer stops.
  /// @param delta minimum error change
  void setMinimumErrorDelta(const Scalar delta)
  {
    _minErrorDelta = delta;
  }
  
  /// Set the maximum number of consecutive stagnating iterations.
  /// If the error does not change significantly for this many iterations, the optimizer stops.
  /// Set to 0 or negative to disable stagnation detection.
  /// @param iterations maximum number of stagnating iterations
  void setMaximumStagnationIterations(const Index iterations)
  {
    _maxStagnationIterations = iterations;
  }
  ...
  Result minimize(const InputVector &initialGuess)
  {
    ...
    bool succeeded = true;
    Scalar lastError = std::numeric_limits<Scalar>::max();
    Index stagnationCount = 0;
    ...
    while((_maxIt <= 0 || iterations < _maxIt) &&
          ...
          stepLen >= _minStepLen &&
          error >= _minError &&
          (_maxStagnationIterations <= 0 || stagnationCount < _maxStagnationIterations) &&
          callbackResult)
    {
      xval -= step;
      objective(xval, fval, jacobian);
      ...
      if(_maxStagnationIterations > 0)
      {
        const Scalar errorDelta = std::abs(error - lastError);
        if(errorDelta < _minErrorDelta)
        {
          ++stagnationCount;
        }
        else
...
private:
  ...
  Scalar _minErrorDelta = static_cast<Scalar>(1e-9);
  Index _maxStagnationIterations = 10;
  Index _verbosity = 0;
